...
var InternalI18nContext = import_react8.default.createContext({
  locale: null,
  format: (_namespace, _component, _key, provided) => provided
});
function useLocale() {
  return (0, import_react8.useContext)(InternalI18nContext).locale;
}
function useInternalI18n(componentName) {
  const { format } = (0, import_react8.useContext)(InternalI18nContext);
...
function useContainerQuery(mapFn, deps) {
  ...
  var elementRef = (0, import_react9.useRef)(null);
  var _a = (0, import_react9.useState)(null), state = _a[0], setState = _a[1];
  var getElement = (0, import_react9.useCallback)(function() {
...
function TabHeaderBar({ onChange, activeTabId, tabs, variant, idNamespace, ariaLabel, ariaLabelledby, i18nStrings }) {
  const headerBarRef = (0, import_react11.useRef)(null);
  const activeTabHeaderRef = (0, import_react11.useRef)(null);
  const inlineStartOverflowButton = (0, import_react11.useRef)(null);
  const i18n = useInternalI18n("tabs");
  const isVisualRefresh = useVisualRefresh();
  const [widthChange, containerRef] = useContainerQuery((rect) => rect.contentBoxWidth);
  const tabRefs = (0, import_react11.useRef)(/* @__PURE__ */ new Map());
  const [horizontalOverflow, setHorizontalOverflow] = (0, import_react11.useState)(false);
  const [inlineStartOverflow, setInlineStartOverflow] = (0, import_react11.useState)(false);
  const [inlineEndOverflow, setInlineEndOverflow] = (0, import_react11.useState)(false);
  ...
  return (
    ...
    import_react11.default.createElement(
      ...
      horizontalOverflow && import_react11.default.createElement(
        "span",
        { ref: inlineStartOverflowButton, className: leftButtonClasses },
        import_react11.default.createElement(InternalButton, { formAction: "none", variant: "icon", iconName: "angle-left", disabled: !inlineStartOverflow, __focusable: true, onClick: () => onPaginationClick(headerBarRef, "backward"), ariaLabel: i18n("i18nStrings.scrollLeftAriaLabel", i18nStrings === null || i18nStrings === void 0 ? void 0 : i18nStrings.scrollLeftAriaLabel) })
      ),
      import_react11.default.createElement("ul", { role: "tablist", className: styles_css_default2["tabs-header-list"], "aria-label": ariaLabel, "aria-labelledby": ariaLabelledby, ref: headerBarRef, onScroll }, tabs.map(renderTabHeader)),
      horizontalOverflow && import_react11.default.createElement(
        "span",
  ...
  function renderTabHeader(tab) {
    ...
    const commonProps = {
      ...
      children: import_react11.default.createElement("span", { className: styles_css_default2["tabs-tab-label"] }, tab.label)
    };
    if (tab.disabled) {
    ...
    if (tab.href) {
      ...
      trigger = import_react11.default.createElement("a", Object.assign({}, anchorProps));
    } else {
      ...
      trigger = import_react11.default.createElement("button", Object.assign({}, buttonProps));
    }
    return import_react11.default.createElement("li", { ref: (element) => tabRefs.current.set(tab.id, element), className: styles_css_default2["tabs-tab"], role: "presentation", key: tab.id }, trigger);
  }
}
...
// node_modules/@cloudscape-design/components/internal/hooks/use-controllable/index.js
function useControllable2(controlledValue, handler, defaultValue2, { componentName, changeHandler, controlledProp }) {
  const isControlled = React4.useState(controlledValue !== void 0)[0];
  if (isDevelopment2) {
    React4.useEffect(() => {
  ...
  const [valueState, setValue] = React4.useState(defaultValue2);
  const [valueHasBeenSet, setValueHasBeenSet] = React4.useState(false);
  const currentUncontrolledValue = valueHasBeenSet ? valueState : defaultValue2;
  const setUncontrolledValue = React4.useCallback((newValue) => {
    setValue(newValue);
...
function Tabs(_a) {
  ...
  const content = () => {
    ...
    const renderContent = (tab) => {
      ...
      return import_react12.default.createElement("div", Object.assign({}, contentAttributes), isContentShown && selectedTab.content);
    };
    return import_react12.default.createElement("div", { className: clsx_m_default(variant === "container" || variant === "stacked" ? styles_css_default2["tabs-container-content-wrapper"] : styles_css_default2["tabs-content-wrapper"], {
      [styles_css_default2["with-paddings"]]: !disableContentPaddings
    }) }, tabs.map(renderContent));
  };
  const header = import_react12.default.createElement(TabHeaderBar, { activeTabId, variant, idNamespace, ariaLabel, ariaLabelledby, tabs, onChange: (changeDetail) => {
    setActiveTabId(changeDetail.activeTabId);
    fireNonCancelableEvent(onChange, changeDetail);
  }, i18nStrings });
  if (variant === "container" || variant === "stacked") {
    return import_react12.default.createElement(InternalContainer, Object.assign({ header, disableHeaderPaddings: true }, baseProps, { className: clsx_m_default(baseProps.className, styles_css_default2.root), __internalRootRef, disableContentPaddings: true, variant: variant === "stacked" ? "stacked" : "default", fitHeight }), content());
  }
...
export {
  ...
  useControllable2 as useControllable,
  Tabs
};
//# sourceMappingURL=chunk-A5O6NEDK.js.map
